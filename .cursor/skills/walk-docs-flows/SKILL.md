...
- Invalid JSON still opens Create plan with an issues banner; valid `assets/json/plan.json` opens Create plan then Finish plan
...
- Live: session is a snapshot; supersets alternate; rest auto-starts after save (countdown +15s / Skip); soft rate Easy→Brutal or Skip for now; all finished → done beat → complete; Finish vs Discard
