...
void main() {
  ...
  test('felt easier stays off when a rating is missing or there is one session',
      () {
    ...
    expect(missingRating.exercises.single.delta, 0);

    final single = service.fold(
      month: DateTime.utc(2026, 8),
      sessions: [
        _session(
          id: 1,
          startedAt: DateTime.utc(2026, 8, 2),
          logs: [_repLog(title: 'squat', reps: [8], weight: 40, difficulty: 2)],
        ),
      ],
    );
    expect(single.exercises.single.feltEasier, isFalse);
  });
}
